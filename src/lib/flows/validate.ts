    ...
    case "send_template": {
      ...
      if (!cfg.template_name?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "template_name",
          message: "Send-template node needs an approved template name.",
        });
      }
      if (!cfg.template_language?.trim()) {
        issues.push({
          severity: "error",
          scope: "node",
          node_key: node.node_key,
          field: "template_language",
          message: "Send-template node needs the template's approved language code.",
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
